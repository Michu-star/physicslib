...
def derivative_state(x, state, last_derivative):
    derivative = np.zeros_like(state)

    # y', ..., y^(n-1)
    derivative[:-1] = state[1:]
    # y^n
    derivative[-1] = last_derivative(x, state)

    return derivative


# These functions solve ODEs of any order
def euler(x, y0, nth_derivative_y):
    dx = np.diff(x)
    n = len(y0)
    ...
    for i in range(len(x) - 1):
        # y, y', ..., y^(n-1)
        y[i + 1] = y[i] + derivative_state(x[i], y[i], nth_derivative_y) * dx[i]

    return y


def rk4(x, y0, nth_derivative_y):
    dx = np.diff(x)
    n = len(y0)
...
def rk4_system(x, y0, state_derivative):
    dx = np.diff(x)
    n = len(y0)
